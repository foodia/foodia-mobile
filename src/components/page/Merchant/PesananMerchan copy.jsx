    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}order/filter?merchant_id=${id}`,
          {
            headers: {
          ...
          <div className="flex my-5 p-2">
            <div
              className={`mr-2 grid justify-items-center ${
                selectedStatus === "approved" ? "text-blue-500 " : ""
              }`}
            ...
            >
              ...
              <div
                className={`w-24 h-0.5 ${
                  selectedStatus === "approved" ? "bg-blue-500 " : "bg-black"
                }`}
              ></div>
            </div>
            <div
              className={`mr-2 grid justify-items-center ${
                selectedStatus === "listMenu" ? "text-blue-500" : ""
              }`}
            ...
            >
              ...
              <div
                className={`w-24 h-0.5 ${
                  selectedStatus === "listMenu" ? "bg-blue-500 " : "bg-black"
                }`}
              ></div>
            </div>
            <div
              className={`mr-2 grid justify-items-center ${
                selectedStatus === "" ? "text-blue-500" : ""
              }`}
            ...
            >
              ...
              <div
                className={`w-24 h-0.5 ${
                  selectedStatus === "" ? "bg-blue-500 " : "bg-black"
                }`}
              ></div>
            </div>
          </div>
          ...
          <div className={`${styles.card}`}>
            {filteredData.map((data) => (
              <CardPesanan
                ...
                to={`/product/${data.id}`}
                img={
                  data.campaign.image_url && data.campaign.image_url
