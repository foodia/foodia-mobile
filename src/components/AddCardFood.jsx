...
const CardFood = (props) => {
    ...
    return (
        <Link href={`${to}`} className="w-full bg-white hover:bg-gray-100  text-black rounded-lg inline-flex items-center px-4 py-2.5 ">
            <div className="flex justify-between w-full">
                <div className="flex">
                    ...
                    <div className="text-left place-items-start">
                        ...
                        <div className=" font-sans text-xs text-gray-500">{name}</div>
                        <div className="flex">
                            <p className="font-sans text-xs text-gray-500 mr-2">qty : {qty}</p>
                            <div className={`font-sans text-xs text-white  rounded-lg  flex justify-center items-center ${status == 'Waiting approval' ? 'bg-blue-600 w-24' :
                                status == 'Approved' ? 'bg-green-500 w-14' :
                                    status == 'Rejected' ? 'bg-red-500 w-14' : 'bg-gray-500 w-14'
                                } `}>
